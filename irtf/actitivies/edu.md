---
title: Internet Research Education
description: Resources for Internet Research Education
published: true
date: 2026-08-05T14:32:50.609Z
tags: irtf, edu
editor: markdown
dateCreated: 2026-08-05T14:32:50.609Z
---

# Internet Research Education

*A community activity of the Internet Research Task Force (IRTF).*

## Overview

The Internet Research Education activity aims to improve access to high-quality educational resources for Internet research and advanced Internet technologies.

Excellent teaching materials already exist across universities, research groups, standards organizations, conferences, and open-source communities. However, these resources often remain difficult to discover, scattered across many websites and repositories, and maintained independently.

The goal of this activity is **not** to replace existing educational efforts or create new textbooks from scratch. Instead, it seeks to identify, curate, organize, and promote reusable educational resources covering Internet research topics, making them easier for educators, students, researchers, and practitioners to discover and reuse.

The catalogue is intentionally organized around **Internet research topics** rather than individual resources. It is **not intended to be exhaustive**. Instead, it aims to identify a curated collection of high-quality educational resources that support teaching, self-study, and research.

Contributions from the Internet research community are warmly encouraged.

---

# Goals

The Education Activity aims to:

- Curate educational resources relevant to Internet research.
- Encourage reuse of teaching material across institutions.
- Promote reproducible laboratory exercises and research artifacts.
- Improve the visibility of emerging Internet research topics in education.
- Facilitate collaboration among educators, researchers, and practitioners.
- Identify gaps where new community-developed educational material would be beneficial.

---

# Internet Research Topics

Each topic should gradually evolve into a curated portal containing:

- Overview
- Learning Resources
- Standards and Research
- Software and Implementations
- Hands-on Laboratories
- Videos and Tutorials
- Related Topics

---

# Internet Foundations

## Internet Architecture

*(Resources to be added.)*

## Naming and DNS

### Learning Resources

- **Mini Internet Project**  
  https://github.com/nsg-ethz/mini_internet_project

Hands-on Internet-scale laboratories covering DNS deployment and operation.

### Standards and Research

- RFC 1034 — Domain Names: Concepts and Facilities
- RFC 1035 — Domain Names: Implementation and Specification

## Internet Measurement

*(Resources to be added.)*

---

# Internet Protocols and Systems

## QUIC

### Learning Resources

- **Computer Networking: Principles, Protocols and Practice**  
  https://4ed.computer-networking.info/syllabus/default/hosts/quic.html

- **Computer Networks: A Systems Approach**  
  https://systemsapproach.org/books/computer-networks/

### Standards and Research

- RFC 9000 — QUIC
- RFC 9001 — Using TLS to Secure QUIC
- RFC 9002 — QUIC Loss Detection and Congestion Control
- RFC 9114 — HTTP/3
- RFC 9221 — QUIC DATAGRAM

### Software and Implementations

*(Resources to be added.)*

### Hands-on Laboratories

*(Resources to be added.)*

### Related Topics

- HTTP/3
- WebTransport
- Media over QUIC (MoQ)

## HTTP/3

*(Resources to be added.)*

## Congestion Control

*(Resources to be added.)*

## Multipath Transport

*(Resources to be added.)*

## Routing

### Learning Resources

- **Mini Internet Project**  
  https://github.com/nsg-ethz/mini_internet_project

Hands-on laboratories covering Internet routing.

## Border Gateway Protocol (BGP)

### Learning Resources

- **Mini Internet Project**  
  https://github.com/nsg-ethz/mini_internet_project

Practical laboratories covering BGP, routing policies, and Internet topology.

### Standards and Research

- RFC 4271 — BGP-4

## Media over QUIC (MoQ)

*(Resources to be added.)*

## WebTransport

*(Resources to be added.)*

---

# Future Internet Architectures

## Information-Centric Networking

### Learning Resources

*(Resources to be added.)*

### Standards and Research

*(Seminal papers, IRTF RFCs, Internet-Drafts, and surveys to be added.)*

### Software and Implementations

- Mini-NDN — https://github.com/named-data/mini-ndn
- ndnSIM — https://ndnsim.net/

### Hands-on Laboratories

*(Resources to be added.)*

## Path-Aware Networking

*(Resources to be added.)*

## Compute-First Networking

*(Resources to be added.)*

## Publish/Subscribe Networking

*(Resources to be added.)*

---

# Distributed Systems

## Distributed Systems

### Learning Resources

- **Distributed Systems: A Systems Approach**  
  https://systemsapproach.org/books/distributed-systems/

## Cloud Computing

*(Resources to be added.)*

## Edge Computing

*(Resources to be added.)*

---

# AI and Internet Systems

- AI Networking
- Distributed AI Training
- AI Inference Systems
- Collective Communication
- AI Agents
- Agent Communication

*(Resources to be added.)*

---

# Internet Security and Privacy

- Internet Security
- Privacy-Enhancing Technologies
- RPKI

*(Resources to be added.)*

---

# Internet Research Methods

- Experimental Methodology
- Reproducible Research
- Network Simulation (ns-3)
- Network Emulation (Mininet, Mini Internet Project, Mini-NDN)
- Benchmarking
- Research Software
- Research Artifacts

---

# Internet Standards and Open Standards

- IETF Standards Process
- Internet-Drafts
- RFCs
- Running Code and Open Source
- Interoperability
- Hackathons

*(Resources to be added.)*

---

# Featured Resources

## Open Textbooks

- Computer Networking: Principles, Protocols and Practice  
  https://4ed.computer-networking.info/

- Systems Approach Book Series  
  https://systemsapproach.org/books-html/

## Community Portals

- ACM SIGCOMM Education Portal  
  https://education.sigcomm.org/

## Laboratory Platforms

- Mini Internet Project
- Mini-NDN
- ns-3
- Mininet
- ndnSIM

---

# Contributing

Contributions are welcome from educators, researchers, students, practitioners, standards organizations, and the open-source community.

Useful additions include:

- Educational resources
- Open textbooks
- RFCs and Internet-Drafts
- Seminal research papers
- Laboratory exercises
- Software and reference implementations
- Tutorials
- Videos
- Summer schools
- Research artifacts
- Corrections and updates

Please keep descriptions concise, objective, and up to date.

---

# Future Directions

Potential future activities include:

- Community-maintained educational resource collections
- Reusable lecture modules
- Community tutorials
- Summer schools
- Reproducible laboratory environments
- Educational best practices
- AI-assisted Internet research education
- Guidance for transforming research artifacts into reusable educational resources
