...
# Process for Handling Incoming Liaison Statements

In short:
* **All incoming statements are expected to be sent to either liaison-coordination@iab.org or the liaison manager** (ideally using the liaison alias address instead of a personal address)
* **Only liaison coordinators or liaison managers should upload** incoming liaison statements to the datatracker Liaison management tool (the secretriat can help in exceptional cases)
  * Check that the statement is addressed to the right group(s), or otherwise communicate this back to the sender before uploading
  * Using "post and send" will then distribute the liaison statement to the respctive IETF groups in the "To" field
* It is recommended for the liaison coordinators or managers to **send a short, informal mail back with the link to the record** to confirm reception
* Any incoming statements that are received on other lists (including statements@ietf.org) should be forwarded to the liaison coordinator list or respective liaison manager for upload
