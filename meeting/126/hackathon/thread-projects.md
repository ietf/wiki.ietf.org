---
title: IETF 126 Hackathon Thread Projects
description: 
published: true
date: 2026-07-08T08:52:47.013Z
tags: 
editor: markdown
dateCreated: 2026-04-14T18:40:10.133Z
---

# IETF 126 Hackathon Thread Projects

IETF 126 Vienna, July 2026.

To avoid taking up too much space on the [main Hackathon page](/meeting/126/hackathon), Thread projects are listed separately here.
...
- **Slack Channel**
For communication among Thread Hackathon participants, particularly remote participants, in the [IETF Slack Workspace](https://join.slack.com/t/ietf/shared_invite/zt-429bk88py-byqcJ4Epp4pa5BSCtEOy4g) we have the [#team-thread-hackathon](https://ietf.slack.com/archives/C07DE1M657E) channel.
...
- **Project Info**
  ...
  - **cBRSKI Device Onboarding.** In the [ANIMA WG](https://datatracker.ietf.org/wg/anima/about/), a lightweight version of the BRSKI device onboarding protocol has been developed: [Constrained BRSKI](https://datatracker.ietf.org/doc/html/draft-ietf-anima-constrained-voucher) or cBRSKI. This protocol is suitable for professional installation use cases, to onboard a large number of new Thread IoT devices onto a network while checking for mutual authenticity and allowing a device manufacturer to be in the loop during the process. There is ongoing implementation work on a [Registrar + MASA](https://github.com/EskoDijk/ot-registrar) and OpenThread (code public soon) to enable cBRSKI. It re-uses Thread MeshCoP mechanisms, like the above project. This work does not require dev boards: the OTNS3 simulator (code coming soon) is used to build and test cBRSKI with virtual OpenThread nodes that connect to real infrastructure on local network and Internet.
  ...
  - **TCP Ack Rate Request.** In normal operation, TCP sends an acknowledgement for every second packet. This is good for efficiency, but causes lower throughput when an extremely contrained sender is only able to have a single packet in flight at a time. The receiver will delay sending its acknowledgement (typically for 200 ms) while it waits for a second packet to arrive, but that second packet will not arrive because the sender will be waiting for the acknowledgement before it can send its next packet. This limits throughput to at most five packets per second. TCP in OpenThread should implement the [TCP ACK Rate Request option (TARR)](https://datatracker.ietf.org/doc/html/draft-ietf-tcpm-ack-rate-request), both for the sending side and the receving side. This allows a sender to signal that it will be sending only one packet at a time, and the receiver will accommodate this by acknowledging each received packet immediately, without waiting for a second packet. This enables data transfer to proceed at one packet per round-trip time, which is the fastest possible rate for a constrained sender that can only have a single packet at a time in flight.
